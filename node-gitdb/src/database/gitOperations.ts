...
export class GitOperations {
    ...
    public getSimpleGit(): SimpleGit {
        console.log(`Creating simple-git instance for mountpoint: ${this.mountPoint}`);
    ...
    public async ensureCheckedOutLatest() {
        ...
        if (existsSync(this.mountPoint)) {
            ...
            try {
                ...
                const result = await gitInstance.clone(this.repo, this.mountPoint, { '--branch': this.branch });
                console.log(result);
    ...
    public async getCurrentBranch(git?: SimpleGit): Promise<string> {
        ...
        try {
            ...
            console.warn('The branch may exist but have no commits. If so, please create an initial commit.');
            throw error;
        }
    }
